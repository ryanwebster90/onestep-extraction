...
class StableDiffusionWBAttack(DiffusionPipeline, TextualInversionLoaderMixin):
    r"""
    Pipeline for computing the "Denoising confidence score" whitebox attack underlined in: 
    https://arxiv.org/abs/2305.08694
    calling pipe(prompt,...) instead returns the whitebox score. Uses the karras wrappers for sampling.
    ...
    def __call__(
        ...
        compute_images: Optional[bool] = False
    ):
        r"""
        ...
        Args:
            ...
            use_karras_sigmas (`bool`, *optional*, defaults to `False`):
                ...
                Karras`.
            compute_images (`bool`, *optional*, defaults to `False`):
                Whether or not to decode the latents to synthesize images. This is normally just for visualization; only the 
                score is used for the whitebox attack.
        ...
        z0 = latents
        # z0 = latents = torch.randn([1,4,64,64], device=device) * sigmas[0]
        
        # 8. Run k-diffusion solver
        latents = self.sampler(model_fn, latents, sigmas)
    
        wb_score = ( (z0 - latents)**2).mean()
        ...
        return image, z0, latents, wb_score
